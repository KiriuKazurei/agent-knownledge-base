...
Skill discovery uses `POST /api/v1/skills/query` and matches the Skill name and description. Activation uses the manifest endpoint, which returns the complete `SKILL.md` plus a relative file manifest; additional files are fetched through a path-confined read-only endpoint. Knowledge query responses can include lightweight `requiredSkills` metadata when a library has a `library_requires_skill` link.
...
- Agent tokens have explicit scopes and optional library allow-lists.
