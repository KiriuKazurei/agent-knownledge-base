...
func (s *Store) GetLibrary(ctx context.Context, id string) (model.Library, error) {
	...
	err := s.DB.QueryRowContext(ctx, "SELECT id,name,description,allow_remote_models,auto_summarize_imports,summary_provider_id,auto_review_agent_submissions,review_provider_id,created_at,updated_at FROM libraries WHERE id=?", id).Scan(&item.ID, &item.Name, &item.Description, &allow, &autoSummarize, &item.SummaryProviderID, &autoReview, &item.ReviewProviderID, &created, &updated)
	if err != nil {
		return item, err
	}
