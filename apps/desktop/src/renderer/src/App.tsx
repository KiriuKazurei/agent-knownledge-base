...
export function App() {
  ...
  const [activeSection, setActiveSection] = useState<'knowledge'|'skills'>('knowledge')
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      {activeSection === 'skills' ? <SkillsWorkspace libraries={libraries.data ?? []} selectedId={selectedSkill} onSelect={setSelectedSkill} /> : <>
    ...
    <aside className="detail-pane" aria-label={activeSection === 'skills' ? 'Skill 详情' : '文档预览'}>
...
function TokenSettings({libraries}:{libraries:LibraryType[]}) {
  const queryClient=useQueryClient();const tokens=useQuery({queryKey:['tokens'],queryFn:client.tokens});const [name,setName]=useState('');const [secret,setSecret]=useState('');const create=useMutation({mutationFn:()=>client.createToken(name,['query','feedback'],libraries.map((item)=>item.id)),onSuccess:(token)=>{setSecret(token.secret??'');setName('');queryClient.invalidateQueries({queryKey:['tokens']})}});const revoke=useMutation({mutationFn:client.revokeToken,onSuccess:()=>queryClient.invalidateQueries({queryKey:['tokens']})})
  return <section><h3>Agent 访问令牌</h3><p className="settings-lead">令牌只显示一次。默认授予查询和相关性反馈，不允许管理知识内容。</p>{secret&&<div className="secret-box"><strong>立即复制此令牌</strong><code>{secret}</code><button onClick={()=>navigator.clipboard.writeText(secret)}>复制</button></div>}<div className="inline-form"><label><span>令牌名称</span><input value={name} onChange={(e)=>setName(e.target.value)} placeholder="例如：本机 Codex"/></label><button className="button primary" onClick={()=>create.mutate()} disabled={!name.trim()}>创建令牌</button></div><div className="token-list">{tokens.data?.map((token:AgentToken)=><div className="token-row" key={token.id}><KeyRound size={17}/><div><strong>{token.name}</strong><span>{token.scopes.join(' · ')} · {new Date(token.createdAt).toLocaleDateString()}</span></div><button className="danger-icon" aria-label={`撤销 ${token.name}`} onClick={()=>revoke.mutate(token.id)}><Trash2 size={17}/></button></div>)}</div></section>
}
...
function PrivacySettings({libraries}:{libraries:LibraryType[]}) { const queryClient=useQueryClient();const update=useMutation({mutationFn:({id,value}:{id:string;value:boolean})=>client.updateLibrary(id,{allowRemoteModels:value}),onSuccess:()=>queryClient.invalidateQueries({queryKey:['libraries']})});return <section><h3>隐私与安全</h3><p className="settings-lead">默认不把知识片段发送到远程模型。密钥保存在 Windows Credential Manager。</p><div className="security-card"><ShieldCheck size={22}/><div><strong>零遥测模式已启用</strong><p>日志仅保存在本机，可由用户主动导出。</p></div></div><h4>逐库远程授权</h4>{libraries.map((library)=><label className="switch-row" key={library.id}><span><strong>{library.name}</strong><small>{library.allowRemoteModels?'允许远程生成':'仅限本地处理'}</small></span><input type="checkbox" checked={library.allowRemoteModels} onChange={(e)=>update.mutate({id:library.id,value:e.target.checked})}/></label>)}</section> }
